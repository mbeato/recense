...
# Compiled adapter CLI paths (D-88: recense.js is now the dispatcher entry point)
...
# Verify all required dist files exist after build (incl. recense.js dispatcher, D-88)
MISSING=0
for f in "$RECENSE_JS" "$SESSION_START_CLI" "$TURN_CAPTURE_CLI" "$STOP_CLI" "$SLEEP_PASS_CLI"; do
    if [[ ! -f "$f" ]]; then
...
# The brain dispatcher form is: recense sleep-pass (run manually: $NODE_BIN $RECENSE_JS sleep-pass)
RECENSE_SLEEP_JS=$SLEEP_PASS_CLI
# ── recense ingest rewire (D-66): to point the hourly job at ingest-cli ──────────
# Dispatcher form: recense ingest (run manually: $NODE_BIN $RECENSE_JS ingest)
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$NODE_BIN $RECENSE_JS hook session-start",
            "timeout": 5
          }
        ]
      }
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$NODE_BIN $RECENSE_JS hook turn-capture",
            "timeout": 5
          }
        ]
      }
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$NODE_BIN $RECENSE_JS hook stop",
            "timeout": 5
          }
        ]
      }
...
echo "    Brain dispatcher: $RECENSE_JS"
