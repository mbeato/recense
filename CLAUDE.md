...
- **Tech stack**: TypeScript engine (better-sqlite3, API embed; LLM extract/judge via the headless `claude -p` transport) — the integration surface (Claude Code hooks) is TS, and v1 has no heavy compute. Python training sidecar bolts on at v3 behind the ModelProvider/ConsolidationSink seams. — Keeps the hot integration path in-process; isolates ML to a separable service.
- **Sleep-pass model stack** (spike 003, current): extract → **headless Haiku** (`claude-haiku-4-5`), judge → **headless Sonnet** (`claude-sonnet-4-6`), both via `claude -p` on the founder's **Max subscription** (NOT direct API, NOT the old local granite+35b default). Billing is flat against the subscription, so marginal write-cost is ~$0 but token usage is real and counts against subscription limits; the `claude -p --output-format json` envelope reports per-call `usage`/`total_cost_usd`. `consolSkipThreshold` gates low-salience turns out of extraction to cut the dominant Haiku cost. Config: `config.ts:620-622`. — Subscription-billed, so cost-benefit is measured in tokens (with retail-$ translation), not direct API spend.
