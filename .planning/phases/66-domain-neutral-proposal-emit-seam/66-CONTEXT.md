...
- **D-02a (amendment, 2026-08-03, recorded during plan verification):** One additional recense-internal column beyond D-02's list: `belief_node_id` — the id of the specific belief-value node `applyDecision` minted for the change. Justification: EMIT-07's "belief moved past the proposal" check (Pitfall 13's literal prescription) is not implementable against the entity node alone; the field is excluded from the D-07 id hash (it is `newId()`-random — including it would falsify EMIT-04 determinism on replay) and is NOT a consumer-schema leak (same recense-internal namespace as `entity_node_id`). Plan-checker reviewed the justification as sound. Flag for the founder in the phase report.
