...
### Phase 999.4: Reader Layer — generated human-readable docs over the brain (BACKLOG — slice validated 2026-06-17)

**Goal:** Productize the validated reader slice (`.planning/reader-layer-SPEC.md`) into a real feature: recense becomes the source of truth, with a generated human-readable doc layer that links inline from prose claims down to atomic facts — retiring Obsidian-as-authoring while keeping a pleasant reading surface. Slice proved the core bet (Sonnet generated a Tonos deep-dive from real facts: 19/19 citations resolve, 0 invented, 100% coverage; reader UI prototyped in `src/viz`, all headless checks green).

**Scope:**
- doc-as-node (`type='doc'`) with memory-lifecycle exemptions (no recall-embedding / eviction / decay / training_eligible); writes routed through the single-writer consolidator.
- facts→doc generation pass (inverted extraction; emits inline `recense://fact/<id>` citations as it cites atoms — citation falls out of generation for free).
- update-in-place: `last_access > generatedAt` staleness detection, `prev_value` diff surfacing, tombstone handling, regen via reverse `edge kind='cites'` lookup for dependent docs. (Id-based refs ⇒ resolved views never stale by construction; only prose drifts.)
- reader UI — promote the prototype (`/doc` route, `reader.js`, Reader/Brain toggle, fact-ref→`selectNode`) from `scripts/reader-slice/` + the `src/viz` slice changes.
- doc→doc corpus graph (Obsidian-style map; `doc_link` edges; subsumes any per-project graph view — decided 2026-06-17).

**Done:** a project doc generates from current facts with 100%-resolving inline citations; clicking a claim focuses its atom in the graph; doc→doc graph navigable; stale citations flagged + regenerable; reads at least as well as the current hand-generated vault deep-dive.

**Depends on:** 999.3 (scope-aware provenance — reliable project→facts scoping), 999.2 (retrieval embeddings — gather breadth; today's slice gather is lexical+entity union), and 999.5 (entity dedup — so docs/graph aren't muddied by fragments). Execution order across backlog: 999.3 → 999.5 → 999.2 → 999.4.

**Requirements:** TBD (formalize at promotion)
**Plans:**
- [ ] TBD (promote with /gsd:review-backlog when ready)

### Phase 999.5: Entity dedup/prune pass (BACKLOG — drafted 2026-06-17)

**Goal:** Scheduled consolidation pass that merges near-duplicate entities and collapses fragments so retrieval and generated docs aren't muddied. Motivation: the reader slice (2026-06-17) surfaced 8+ near-duplicate "brain-memory" entities and a "tonos" / "Tonos daily eval pipeline" split, max edge degree ~15 — the entity layer is too fragmented to cleanly assemble "all facts for project X", which directly degrades reader gather quality (999.4) and any doc→doc graph.

**Done:** a repeatable prune/dedup pass merges near-duplicate entities (value + embedding similarity above threshold, origin-guarded) into a canonical node, rewires their edges, and tombstones the duplicates without losing provenance; entity fragmentation for a sample project (e.g. brain-memory) measurably drops; no regression to recall accuracy.

**Depends on:** 999.3 (`node_scope` lands first — scope-aware merging). Prerequisite for 999.4's doc→doc graph.
