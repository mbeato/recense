...
## Current Milestone: v9.0 Memory Quality
...
**v9.0 Memory Quality (open).** Scoped requirements live in `REQUIREMENTS.md`; phases in `ROADMAP.md` (46–50). Headline groups:

- [ ] **RECON-01..04** — reconsolidation candidate broadening (resolves backlog 999.2): union candidate generation (entity/subject-keyed + BM25 + dense) feeding the judge; judge fires on real KU contradictions; KU belief-correction improves; EVAL-02 clean-case correctness holds.
- [ ] **RETR-01..04** — hybrid BM25+dense fusion on the LLM-free online path; tunable weight; R@5/R@10 lift on LoCoMo; hot-path latency within budget.
- [ ] **HARD-01..04** — close C-2 self-confirmation; M-5 immediate() write txns; M-9 schema-version read-first guard; L-2 embedding model/dims stamp+assert.
- [ ] **SCALE-01..02** — vectorlite ANN-vs-brute-force crossover + go/no-go; bi-temporal/supersedes vs tombstone recommendation (spike; may defer).
- [ ] **GATE-01..03** (discharges deferred Phase 43) — automated regression gate; thresholds on accuracy/latency/token + belief-correction + recall; baseline = v9.0-final; full eval re-run with before/after deltas + judge-fires proof; `docs/evals.md` updated.
...
| Defer Phase 43 (Eval Regression Gates) rather than block the v8.0 close (2026-06-26) | The v8.0-final numbers are recorded; the gate freezes them but is independent work, and Phase 42's reset-window eval gated it. Closing with a documented gap + carrying GATE-01/02/03 forward beats holding the milestone | ⚠️ Revisit — numbers can silently regress until the gate lands; first item of the next milestone |
