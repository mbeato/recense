...
- [x] 57-07-PLAN.md — Stage-2 founder checkpoint: full-system live tune (motion + floors + bloom + exposure), ratchet-lock all provisional values + tighten D-12 invariants + per-layer evidence + 2558-suite gate [wave 6, checkpoint] (VIZ-PAL-02, VIZ-PAL-06, VIZ-PAL-07, VIZ-PAL-05)
...
- [x] 58-03-PLAN.md — Diegetic SDF schema labels (top-N by member count, appear-on-approach) + LABEL_COLOR lock [wave 2]

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 58-04-PLAN.md — Focus-tier matcap mix (gen-matcap script + onBeforeCompile + damped fade) + 32-seg focus geo [wave 3]

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 58-05-PLAN.md — Stage-1 founder checkpoint: LOOK (haze + labels + matcap; troika keep/kill; no-CDN check) [wave 4]

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 58-06-PLAN.md — Unified damped interruptible camera system (orbit-then-dolly focus, transition re-drive) [wave 5]
...
- [x] 58-08-PLAN.md — Stage-2 founder checkpoint: MOTION (hover/flight/transition/dim-fog) + before/after fps [wave 7]
...
- [x] 59-01-PLAN.md — Token foundation: HUD_CSS_TOKENS + runtime CSS-var emission + @font-face + D-11/D-10 invariants

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 59-02-PLAN.md — Exhaustive CSS token migration (89 literals) + glass reskin of detail/settings/reader/tooltip + D-14 scan

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 59-03-PLAN.md — Vanilla ⌘K palette (Nodes BM25 / Topics / Commands, select-and-go) + search/topics/hud refactors
...
- [x] 59-05-PLAN.md — Auto-recede: idle + camera-flight + focus → opacity ghost; camera/stats state exposure
...
- [x] 60-03-PLAN.md — Full-window #stats-view takeover shell (glass-free) + tabs/range/refresh + Usage tab render + app.js wiring
...
- [x] 60-06-PLAN.md — Closing founder live-install sign-off checkpoint
...
- [x] 61-01-PLAN.md — Glass reskin of #index-panel + #index-reopen and new tree-row CSS classes (D-02, D-08)
- [x] 61-02-PLAN.md — Corpus graph focus/dim + hover subtree parity + tiered labels + focus-driven chapters; new focusCorpusProject/setCorpusProjectExpanded hooks (D-04..D-07)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 61-03-PLAN.md — Index collapsible tree with chevron + count badge; project-name click focuses the graph (D-01, D-03)
...
**Round-3 Gap Closure (61-VERIFICATION predicate-drift WR-01/03/04 + 61-UAT GAP-9 floating panel)**

*R3 Wave 1*
- [x] 61-15-PLAN.md — WR-01/03/04: unify the recognized-project definition (server ships the set, client consumes it), null-guard chapter visibility + label tiering, make syncCorpusFocus the single writer of the index active row (+ WR-05/WR-06)
...
*R4 Wave 1*
- [x] 61-17-PLAN.md — GAP-7: defer the MAX_ZOOM clamp until the focus/unfocus animation completes (no overshoot, no mid-transition snap); GAP-8: apply a shared humanTitle() UUID guard to the /graph?type=doc node label (never a raw UUID on hover)

*R4 Wave 2 (blocked on R4 Wave 1 — sequenced so the founder verifies a fully-fixed build)*
- [x] 61-18-PLAN.md — GAP-10: rework the index from a floating overlay into a docked LEFT column outside the graph (reinstated `.index-docked #corpus-graph` reflow, drag machinery removed); closing founder live-install sign-off
