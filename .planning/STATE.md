...
- CORRECTION: Phase 25 entity dedup is ALREADY done + live (ran 2026-06-18, opt-in `recense dedup-entities` CLI). It is NOT the lever for the Max fragmentation — a dry-run confirms "Max"/"Max (design lead)" are not merge candidates (distinct surface forms below threshold), and the anchor-union (typedAnchorPoolK=20) already handles that fragmentation at recall (the reason coverage hit 92%). The dedup CLI is run-once / not sleep-wired; a re-run now would merge ~30 accumulated EXACT-duplicate nodes (cosine=1.0) — general hygiene, does not move typed-recall coverage.
