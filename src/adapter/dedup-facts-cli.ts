/**
 * dedup-facts-cli — recense dedup-facts (Phase 26, Plan 26-08).
 *
 * Opt-in, manual CLI subcommand wrapping the FactDedup engine. Default mode is
 ...
 *
 * This is a direct mirror of dedup-entities-cli.ts, replacing EntityDedup with
 * FactDedup (D-05 pollution exclusion + fact-node specifics, Plan 26-08).
 *
 ...
 *  T-26-19  --dry-run is the DEFAULT; real write requires explicit --no-dry-run;
 *           NOT wired into the hourly sleep pass ([[gated-live-write-needs-real-offswitch]]).
 *
 * Entry point:
 *  - Spawned by recense.ts dispatcher (`recense dedup-facts [--dry-run] [--threshold N] [--db PATH]`)
...
import { FactDedup, type MergeCluster } from '../consolidation/fact-dedup';
import { acquireLock, releaseLock } from './lockfile';
import { resolveDbPath as resolveSharedDbPath } from './runtime-config';

const LOG_PATH = '/tmp/recense-dedup-facts.log';

/** Append a timestamped line to the log file. */
const fileLog = (msg: string): void =>
  appendFileSync(LOG_PATH, `[${new Date().toISOString()}] dedup-facts: ${msg}\n`);
...
export function printDryRun(clusters: MergeCluster[]): void {
  ...
  process.stdout.write('recense dedup-facts — DRY RUN (nothing written)\n\n');
...
async function main(): Promise<void> {
  ...
  if (isDryRun) {
    ...
    if (!dbPath) {
      process.stderr.write(
        'recense dedup-facts: No DB path (--db <path> or RECENSE_DB env var) — exiting\n',
      );
      process.exit(0);
    }
    ...
    try {
      ...
      const dedup = new FactDedup(db, store, sink, realClock, config);
  ...
  if (!acquireLock()) {
    process.stderr.write('recense dedup-facts: Lock held by another process — exiting\n');
    process.exit(0);
  }

  let db: Database.Database | undefined;
  try {
    fileLog('dedup-facts real run starting');
    ...
    process.stdout.write(
      `dedup-facts: ${result.mergedClusters} cluster(s) merged, ${result.tombstoned} node(s) tombstoned\n`,
    );
    fileLog(
...
if (require.main === module) {
  main().catch(err => {
    appendFileSync(LOG_PATH, `[${new Date().toISOString()}] dedup-facts FATAL: ${err}\n`);
    releaseLock();
    process.exit(1);
  });
}
