# Recense: headless Linux server-mode guide (SERVE-03)

This guide walks through deploying recense on a headless Linux host so it is
...
1. **Clone and build**
   ...
   ```sh
   git clone https://github.com/yourname/recense.git /home/user/recense
   cd /home/user/recense
   npm ci
   npm run build
   ```
...
2. **BYO API keys in sleep.env**
   ...
   Create `~/.config/recense/sleep.env` (the file `recense init` writes and the
   systemd units source):

   ```sh
   mkdir -p ~/.config/recense
   cat > ~/.config/recense/sleep.env <<'EOF'
   ANTHROPIC_API_KEY=sk-ant-...
   OPENAI_API_KEY=sk-...
   RECENSE_DB=/home/user/.config/recense/recense.db
   EOF
   chmod 600 ~/.config/recense/sleep.env
   ```
...
4. **Pin `RECENSE_NODE_BIN`**
   ...
   `recense init` (step 3) already wrote `RECENSE_NODE_BIN` into sleep.env. Verify
   the stored value matches the doctor output:

   ```sh
   grep '^RECENSE_NODE_BIN=' ~/.config/recense/sleep.env
   ```

   If it differs, edit that line **in place** — do not append a second
   `RECENSE_NODE_BIN=` line (the systemd section below extracts the value with
   grep; a duplicate key would yield a two-line value and corrupt the generated unit):

   ```sh
   sed -i "s|^RECENSE_NODE_BIN=.*|RECENSE_NODE_BIN=/home/user/.nvm/versions/node/v22.x.x/bin/node|" ~/.config/recense/sleep.env
   ```
...
`recense serve` binds to **127.0.0.1 by default** — requests from outside the machine
are refused before they reach the auth gate. Remote exposure requires an explicit flag:

```sh
recense serve --host 0.0.0.0 --port 7701
```
...
`recense serve` speaks **plain HTTP**. Put Caddy (or nginx) in front for TLS. Caddy
...
recense sees plain HTTP on 127.0.0.1:7701. Caddy handles TLS termination, cert
...
The unit sets `Environment=RECENSE_NODE_BIN=<path>` so the pinned binary is
...
The scheduler runs `recense scheduler run` — an in-process croner job that fires every
...
From your laptop (replace `recense.example.com` with your domain or IP):

**Health check (no token required):**

```sh
curl -s https://recense.example.com/health
...
curl -s -o /dev/null -w "%{http_code}" \
  -X POST https://recense.example.com/v1/search \
...
## Verifying the full setup with `recense doctor`
