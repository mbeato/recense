...
**No-op bug (found + fixed 2026-06-19, commit d517349):** the consolidator judges each claim via `provider.judgeBatch([1 item])` (consolidator.ts:651), *not* `judge()`. The original `TwoTierJudge.judgeBatch` delegated wholesale to Sonnet, so the cheap Haiku tier never ran — two-tier was a complete no-op in production. An earlier "byte-identical, 0/17 mismatches" claim was therefore baseline-vs-baseline and is void. The bug was caught only by the deterministic `getTwoTierStats()` escalation counter reporting `cheap_calls=0` on a clean run; unit tests and a `judge()`-path engagement check both passed falsely (production doesn't call `judge()`). `judgeBatch` now does real per-item triage.

**Validation (EVAL-02, post-fix, 2026-06-19):** engagement confirmed (`cheap_calls` > 0 every run, escalation 35–50% on this contradict-heavy set). Belief-correction across clean runs: two-tier **84.6% × 4 (rock-solid)**, baseline 84.6% with real-noise dips to 76.9% — two-tier is at least as accurate and *more stable*. Zero new missed contradictions. On the hard low-cosine adversarial set (built to stress Haiku suppression), real two-tier scored **100% (7/7)** — no suppression observed. One two-tier run degraded to 46.2% via a headless-timeout that dropped half the claims; baseline hit the same failure mode (69.2%) — infrastructure, not the lever. Non-determinism (no temp-0) means each cell is one sample; the *pattern* across ~9 runs is consistent. Parallel eval runs corrupt (headless concurrency → empty extraction → silent 0%); replicates must run serial.

**Measured economics (per claim, Anthropic list prices):** baseline Sonnet judge $0.00705/claim; two-tier Haiku triage $0.00432/claim (39% cheaper than the Sonnet call it replaces) + Sonnet $0.00619 only on escalated claims → **break-even escalation rate ≈ 44%**. On EVAL-02 (45% escalation) Sonnet $ dropped 60% but total was flat (sat at break-even). Projected at realistic escalation (~1–5%, since live `contradict` ≈0.3% of judged claims): **judge cost −34 to −38%** (≈ −14 to −30% of total write cost depending on the judge/extract split). The net-saving magnitude is a projection from measured per-call costs, not a direct measurement — EVAL-02 is too contradict-heavy to measure it directly. Enabled in production via `RECENSE_TWO_TIER_JUDGE=1`; the live escalation counter is the production canary. Results: `scripts/eval/results/{real-twotier,hard-real-twotier,cost-std-baseline,rep2-*}.json`.
