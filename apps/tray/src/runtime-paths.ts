 ...
 *
 * Security (T-16-04): reads ONLY the BRAIN_MEMORY_NODE_BIN and
 * BRAIN_MEMORY_SLEEP_JS lines via targeted regex; never sources or echoes
...
/**
 * Default location of the sleep.env file written by `brain init` (chmod-600).
 ...
 *   1. BRAIN_MEMORY_NODE_BIN env var — trimmed; wins if non-empty
 *   2. BRAIN_MEMORY_NODE_BIN= line in sleep.env — surrounding quotes stripped
...
export function resolveNodeBin(): string {
  ...
  const fromEnv = process.env['BRAIN_MEMORY_NODE_BIN'];
  if (fromEnv && fromEnv.trim()) return fromEnv.trim();

  // 2. Parse BRAIN_MEMORY_NODE_BIN from sleep.env
 ...
 *
 * Precedence:
 *   1. BRAIN_MEMORY_BRAIN_JS env var — explicit override
 *   2. Sibling brain.js of BRAIN_MEMORY_SLEEP_JS from sleep.env:
 *      dirname(BRAIN_MEMORY_SLEEP_JS) + '/brain.js'
 *   3. undefined — path unresolvable; caller must error
 *
 * The sleep.env line BRAIN_MEMORY_SLEEP_JS points at e.g.:
...
export function resolveBrainJs(): string | undefined {
  ...
  const fromEnv = process.env['BRAIN_MEMORY_BRAIN_JS'];
  if (fromEnv && fromEnv.trim()) return fromEnv.trim();

  // 2. Parse BRAIN_MEMORY_SLEEP_JS from sleep.env, derive sibling brain.js
  const envFile = sleepEnvPath();
  try {
    if (existsSync(envFile)) {
      for (const line of readFileSync(envFile, 'utf-8').split('\n')) {
        const m = line.match(/^\s*BRAIN_MEMORY_SLEEP_JS\s*=\s*(.+?)\s*$/);
        if (m) {
          const val = m[1]!.replace(/^['"]|['"]$/g, '').trim();
