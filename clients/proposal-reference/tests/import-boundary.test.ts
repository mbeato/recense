 ...
 * clients/proposal-reference/ including this tests/ subdirectory (D-04:
 * strengthens the guard, and means the adapter's in-dir tests are
 * HTTP/fixture-only too — the test itself must not import src/).
 *
 * Hardened per review WR-02:
...
/**
 * Non-vacuousness floor (guard the guard): the directory holds 7 TypeScript
 * files today. If the walk ever returns fewer than this, the scan itself is
 * broken (or the tree was restructured) and the guard must fail loudly
