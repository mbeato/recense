 ...
 *   - schema_version meta stamps SCHEMA_VERSION (8) after initSchema
...
describe('activation_trace table', () => {
  ...
  it('schema_version meta equals SCHEMA_VERSION (8) after initSchema', () => {
    const db = new Database(':memory:');
    initSchema(db);
    const row = db
  ...
  it('SCHEMA_VERSION constant equals 8', () => {
    expect(SCHEMA_VERSION).toBe(8);
  });
});
